...
int RS_first = 0b00000000;
int RS_last = 0b00000000;
...
void setup() {
  ...
  pinMode(SO_latchPin, OUTPUT);
  pinMode(SO_clockPin, OUTPUT);
  pinMode(SO_dataPin, OUTPUT);


  //shifts out the initial relay state on bootup
  digitalWrite(SO_clockPin, LOW);
  digitalWrite(SO_latchPin, LOW);
  shiftOut(SO_dataPin, SO_clockPin, LSBFIRST, 0b01111111); //relays 9-16
  shiftOut(SO_dataPin, SO_clockPin, LSBFIRST, 0b10000000); //relays 1-8
  digitalWrite(SO_latchPin, HIGH);
...
void update_RS() {
  if (RelayState != last_RelayState) {
    ...
    char firstbyte[9];
    RelayState.substring(0, 8).toCharArray(firstbyte, 9);
    char lastbyte[9];
    RelayState.substring(8, 16).toCharArray(lastbyte, 9);
    RS_first = strtol(firstbyte, NULL, 2);
    RS_last = strtol(lastbyte, NULL, 2);


    digitalWrite(SO_clockPin, LOW);
    digitalWrite(SO_latchPin, LOW);
    shiftOut(SO_dataPin, SO_clockPin, LSBFIRST, RS_last); //relays 9-16
    shiftOut(SO_dataPin, SO_clockPin, LSBFIRST, RS_first); //relays 1-8
    digitalWrite(SO_latchPin, HIGH);
...
void getSensorData() {
  int timenow = now();
  if (timenow > last_sensor_read + 2) {

    int chk1 = DHT.read22(DHT22_PIN_1);
    if (chk1 == DHTLIB_OK) {
      t1 = (DHT.temperature * 1.8) + 32;
      h1 = DHT.humidity;
    } else {
      t1 = 0.00;
      h1 = 0.00;
    ...
    int chk2 = DHT.read22(DHT22_PIN_2);
    if (chk2 == DHTLIB_OK) {
      t2 = (DHT.temperature * 1.8) + 32;
      h2 = DHT.humidity;
    } else {
      t2 = 0.00;
      h2 = 0.00;
